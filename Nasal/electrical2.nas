#Tucano-T-27 electrical system.
    # Tucano-T-27 external-battery-switch assumed always on
    # Tucano-T-27 external pwr assumed never connected
...
var init_electrical = func {
    ...
    setprop("sim/model/Tucano-T-27/controls/switches/inverter", 1);
...
AlternatorClass.apply_load = func( amps, dt, src ) {
    var rpm = getprop(src);
    var factor = 0;
    # Tucano-T-27 APU can have 0 rpm
    if (rpm > 0) {
...
var update_virtual_bus = func( dt ) {
    ...
    var master_inv        = getprop("sim/model/Tucano-T-27/controls/switches/inverter");
    var L_gen_volts       = 0.0;
    var R_gen_volts       = 0.0;
    ...
    if (master_alt and !eng_outof) {
        L_gen_volts = alternator.get_output_volts("sim/model/Tucano-T-27/engines/engine[0]/n2");
    }
    if (master_alt1 and !eng_outof1) {
        R_gen_volts = alternator.get_output_volts("sim/model/Tucano-T-27/engines/engine[1]/n2");
    }
    if(master_apu and getprop("controls/APU/generator-serviceable")) {
        APU_gen_volts = alternator.get_output_volts("sim/model/Tucano-T-27/systems/apu/rpm-norm");
    }
...
var DC_ESSEN_bus = func() {
	...
	if((getprop("controls/APU/off-start-switch") or (getprop("sim/model/Tucano-T-27/controls/engines/engine[0]/throttle") >= 0.03) or (getprop("sim/model/Tucano-T-27/controls/engines/engine[1]/throttle") >= 0.03)) and (getprop("consumables/fuel/tank[1]/level-gal_us") > 1.56) and (!getprop("systems/Tucano-T-27-fuel/tank[0]/boost-pump[0]")) and (!getprop("systems/Tucano-T-27-fuel/tank[1]/boost-pump[0]")) and (getprop("controls/fuel/tank[1]/boost-pump-serviceable[1]")) and (DC_ESSEN_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[1]/boost-pump[1]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[1]/boost-pump[1]", 0);
	}
	# APU starter
	if(getprop("controls/APU/off-start-switch") and getprop("controls/APU/serviceable")) {
		...
		if(getprop("sim/model/Tucano-T-27/systems/apu/rpm-norm") < 60) { load += 120.0; }
	} else { setprop("systems/electrical/outputs/apu-start-system", 0.0); }
	if(getprop("sim/model/Tucano-T-27/controls/fuel/cross-feed-sw") and (DC_ESSEN_bus_volts >= 20)) {
		#setprop("systems/Tucano-T-27-fuel/cross-feed-valve", 1);
		...
		#setprop("systems/Tucano-T-27-fuel/cross-feed-valve", 0);
	...
	if(getprop("sim/model/Tucano-T-27/controls/fuel/tk-gate-open") and (getprop("consumables/fuel/tank[1]/level-gal_us") < 203.1) and (getprop("consumables/fuel/tank[2]/level-gal_us") < 203.1) and (DC_ESSEN_bus_volts >= 20)) {
		#setprop("systems/Tucano-T-27-fuel/tk-gate-valve", 1);
	...
	for(var i=0; i<4; i+=1) {
		if(getprop("systems/Tucano-T-27-fuel/tank["~i~"]/fill-valve")) { int_tanks_filled += 1; }
	}
	# external wing tanks
	if((int_tanks_filled > 0) and getprop("controls/fuel/tank[4]/boost-pump[0]") and !getprop("systems/refuel/receiver-lever") and getprop("controls/fuel/tank[4]/boost-pump-serviceable[0]") and (getprop("consumables/fuel/tank[4]/level-gal_us") > 1.56) and (getprop("systems/bleed-air/psi") > 7) and (DC_ESSEN_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[4]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[4]/boost-pump[0]", 0);
	}
	...
	if((int_tanks_filled > 0) and getprop("controls/fuel/tank[4]/boost-pump[0]") and !getprop("systems/refuel/receiver-lever") and getprop("controls/fuel/tank[6]/boost-pump-serviceable[0]") and (getprop("consumables/fuel/tank[6]/level-gal_us") > 1.56) and (getprop("systems/bleed-air/psi") > 7) and (DC_ESSEN_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[6]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[6]/boost-pump[0]", 0);
	}
	# external fuselage tank
	if((int_tanks_filled > 0) and getprop("controls/fuel/tank[5]/boost-pump[0]") and !getprop("systems/refuel/receiver-lever") and getprop("controls/fuel/tank[5]/boost-pump-serviceable[0]") and (getprop("consumables/fuel/tank[5]/level-gal_us") > 1.56) and !getprop("systems/Tucano-T-27-fuel/tank[4]/boost-pump[0]") and !getprop("systems/Tucano-T-27-fuel/tank[6]/boost-pump[0]") and (getprop("systems/bleed-air/psi") > 7) and (DC_ESSEN_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[5]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[5]/boost-pump[0]", 0);
	}
	# left and right gravity valve, in case of wing pump failure
	if((getprop("consumables/fuel/tank[1]/level-gal_us") < 93.75) and (getprop("consumables/fuel/tank[0]/level-gal_us") > 2.34) and (!getprop("systems/Tucano-T-27-fuel/tank[0]/boost-pump[0]")) and (DC_ESSEN_bus_volts >= 20)) {
		#setprop("systems/Tucano-T-27-fuel/l-gravity-valve", 1);
		setprop("controls/fuel/tank[0]/to_tank", 1);
		load += 0.2;
	} else {
		setprop("controls/fuel/tank[0]/to_tank", -1);
	}
	if((getprop("consumables/fuel/tank[2]/level-gal_us") < 93.75) and (getprop("consumables/fuel/tank[3]/level-gal_us") > 2.34) and (!getprop("systems/Tucano-T-27-fuel/tank[3]/boost-pump[0]")) and (DC_ESSEN_bus_volts >= 20)) {
		#setprop("systems/Tucano-T-27-fuel/r-gravity-valve", 1);
...
var L_DC_bus = func() {
	...
	if(!getprop("controls/fuel/tank[0]/fill-dis") and getprop("controls/fuel/tank[0]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[0]/level-gal_us") < 246) and (L_DC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[0]/fill-valve", 1);
		load += 0.2;
	} elsif((getprop("systems/Tucano-T-27-fuel/tank[0]/fill-valve") and (getprop("consumables/fuel/tank[0]/level-gal_us") > 308)) or getprop("controls/fuel/tank[0]/fill-dis") or !getprop("controls/fuel/tank[0]/fill-valve-serviceable") or (L_DC_bus_volts < 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[0]/fill-valve", 0);
	}
	if(!getprop("controls/fuel/tank[1]/fill-dis") and getprop("controls/fuel/tank[1]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[1]/level-gal_us") < 446) and (L_DC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[1]/fill-valve", 1);
		load += 0.2;
	} elsif((getprop("systems/Tucano-T-27-fuel/tank[1]/fill-valve") and (getprop("consumables/fuel/tank[1]/level-gal_us") > 508)) or getprop("controls/fuel/tank[1]/fill-dis") or !getprop("controls/fuel/tank[1]/fill-valve-serviceable") or (L_DC_bus_volts < 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[1]/fill-valve", 0);
	}
	if(!getprop("controls/fuel/tank[2]/fill-dis") and getprop("controls/fuel/tank[2]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[2]/level-gal_us") < 446) and (L_DC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[2]/fill-valve", 1);
		load += 0.2;
	} elsif((getprop("systems/Tucano-T-27-fuel/tank[2]/fill-valve") and (getprop("consumables/fuel/tank[2]/level-gal_us") > 508)) or getprop("controls/fuel/tank[2]/fill-dis") or !getprop("controls/fuel/tank[2]/fill-valve-serviceable") or (L_DC_bus_volts < 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[2]/fill-valve", 0);
	}
	if(!getprop("controls/fuel/tank[3]/fill-dis") and getprop("controls/fuel/tank[3]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[3]/level-gal_us") < 246) and (L_DC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[3]/fill-valve", 1);
		load += 0.2;
	} elsif((getprop("systems/Tucano-T-27-fuel/tank[3]/fill-valve") and (getprop("consumables/fuel/tank[3]/level-gal_us") > 308)) or getprop("controls/fuel/tank[3]/fill-dis") or !getprop("controls/fuel/tank[3]/fill-valve-serviceable") or (L_DC_bus_volts < 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[3]/fill-valve", 0);
	}
	if(getprop("controls/fuel/tank[4]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[4]/level-gal_us") < 595) and (L_DC_bus_volts >= 20) and (getprop("consumables/fuel/tank[4]/capacity-gal_us") == 600)) {
		setprop("systems/Tucano-T-27-fuel/tank[4]/fill-valve", 1);
		load += 0.2;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[4]/fill-valve", 0);
	}
	if(getprop("controls/fuel/tank[5]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[5]/level-gal_us") < 595) and (L_DC_bus_volts >= 20) and (getprop("consumables/fuel/tank[5]/capacity-gal_us") == 600)) {
		setprop("systems/Tucano-T-27-fuel/tank[5]/fill-valve", 1);
		load += 0.2;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[5]/fill-valve", 0);
	}
	if(getprop("controls/fuel/tank[6]/fill-valve-serviceable") and (getprop("consumables/fuel/tank[6]/level-gal_us") < 595) and (L_DC_bus_volts >= 20) and (getprop("consumables/fuel/tank[6]/capacity-gal_us") == 600)) {
		setprop("systems/Tucano-T-27-fuel/tank[6]/fill-valve", 1);
		load += 0.2;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[6]/fill-valve", 0);
	}
	return load;
}
...
var L_AC_bus = func() {
	...
	if(getprop("controls/fuel/tank[0]/boost-pump[0]") and (getprop("controls/fuel/tank[0]/boost-pump-serviceable[0]")) and (getprop("consumables/fuel/tank[0]/level-gal_us") > 1.56) and (L_AC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[0]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[0]/boost-pump[0]", 0);
	}
	if(getprop("controls/fuel/tank[1]/boost-pump[0]") and (getprop("controls/fuel/tank[1]/boost-pump-serviceable[0]")) and (getprop("consumables/fuel/tank[1]/level-gal_us") > 1.56) and (L_AC_bus_volts >= 20) and !getprop("systems/Tucano-T-27-fuel/tank[0]/boost-pump[0]")) {
		setprop("systems/Tucano-T-27-fuel/tank[1]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[1]/boost-pump[0]", 0);
	}
	return load;
}
...
var R_AC_bus = func() {
	...
	var hud_mode = getprop("sim/model/Tucano-T-27/controls/hud/mode-selector");
	if (hud_mode > 0) {
	...
	if(getprop("controls/fuel/tank[3]/boost-pump[0]") and (getprop("controls/fuel/tank[3]/boost-pump-serviceable[0]")) and (getprop("consumables/fuel/tank[3]/level-gal_us") > 1.56) and (R_AC_bus_volts >= 20)) {
		setprop("systems/Tucano-T-27-fuel/tank[3]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[3]/boost-pump[0]", 0);
	}
	if(getprop("controls/fuel/tank[2]/boost-pump[0]") and (getprop("controls/fuel/tank[2]/boost-pump-serviceable[0]")) and (getprop("consumables/fuel/tank[2]/level-gal_us") > 1.56) and (R_AC_bus_volts >= 20) and !getprop("systems/Tucano-T-27-fuel/tank[3]/boost-pump[0]")) {
		setprop("systems/Tucano-T-27-fuel/tank[2]/boost-pump[0]", 1);
		load += 15.0;
	} else {
		setprop("systems/Tucano-T-27-fuel/tank[2]/boost-pump[0]", 0);
	}
	return load;
}
...
var AC_ESSEN_bus = func() {
	...
	#if((getprop("systems/bleed-air/ats-valve[0]") and (getprop("sim/model/Tucano-T-27/controls/engines/engine[0]/starter-switch-position") == 1)) or (getprop("systems/bleed-air/ats-valve[1]") and (getprop("sim/model/Tucano-T-27/controls/engines/engine[1]/starter-switch-position") == 1))) {
	#	setprop("systems/electrical/outputs/engines-ignitors", AC_ESSEN_bus_volts);
	#	load += 50.0;
...
var check_bleed_air= func {
	...
	var p0 = getprop("sim/model/Tucano-T-27/engines/engine[0]/n2") * 0.67;
	var p1 = getprop("sim/model/Tucano-T-27/engines/engine[1]/n2") * 0.67;
	# APU provide bleed air psi > 50 psi up to an altitude of 10'000 ft @ standard day and 100% RPM
	var p2 = (getprop("environment/pressure-inhg")*2.13538+6.1093)*(getprop("sim/model/Tucano-T-27/systems/apu/rpm-norm")/100);
	var bleed_air = p0;
...
var inverter_switch = func {
    var inv_pos = props.globals.getNode("sim/model/Tucano-T-27/controls/switches/inverter", 1);
...
var nav_lights_switcher = func {
    var flash = props.globals.getNode("sim/model/Tucano-T-27/controls/lighting/nav-lights-flash", 1);
    var s_pos = props.globals.getNode("sim/model/Tucano-T-27/controls/lighting/nav-lights-switch", 1);
...
var land_lights_switcher = func {
    var s_pos = props.globals.getNode("sim/model/Tucano-T-27/controls/lighting/land-lights-switch", 1);
    var pos = s_pos.getValue();
    var input = arg[0];
    if ( input == 1 ) {
        if ( pos == 0 ) {
